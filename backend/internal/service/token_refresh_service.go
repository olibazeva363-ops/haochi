...
func (s *TokenRefreshService) postRefreshActions(ctx context.Context, account *Account) {
	...
	if IsClaudeSKImportAccount(account) && account.Status == StatusError {
		if clearErr := s.accountRepo.ClearError(ctx, account.ID); clearErr != nil {
			...
			slog.Info("token_refresh.cleared_sk_import_error", "account_id", account.ID)
		}
		if setErr := s.accountRepo.SetSchedulable(ctx, account.ID, true); setErr != nil {
			slog.Warn("token_refresh.enable_sk_import_schedulable_failed",
				"account_id", account.ID,
				"error", setErr,
			)
		} else {
			account.Schedulable = true
			slog.Info("token_refresh.enabled_sk_import_schedulable", "account_id", account.ID)
			s.notifyAccountSchedulingBlockCleared(account.ID)
		}
	}
