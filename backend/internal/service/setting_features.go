...
func (s *SettingService) defaultAccountSchedulingThresholds() map[string]int {
	thresholds := defaultAccountSchedulingThresholds()
	// Existing deployments used a config-only Anthropic guard. Use it only
	// before the administrator saves the unified threshold setting; explicit
	// platform settings and per-account overrides retain upstream precedence.
	if s != nil && s.cfg != nil {
		threshold := s.cfg.RateLimit.AnthropicUsagePauseThresholdPercent
		if threshold > 0 && threshold < 100 {
			thresholds[PlatformAnthropic] = threshold
		}
	}
	return thresholds
}
...
func (s *SettingService) GetAccountSchedulingThresholds(ctx context.Context) map[string]int {
	if s == nil || s.settingRepo == nil {
		return s.defaultAccountSchedulingThresholds()
	}
	...
	result, err, _ := accountSchedulingThresholdsSF.Do(SettingKeyAccountSchedulingThresholds, func() (any, error) {
		...
		thresholds := s.defaultAccountSchedulingThresholds()
