...
func (s *AccountTestService) recoverClaudeSKImportTokenForTest(ctx context.Context, account *Account, responseBody []byte) (string, error) {
	if s == nil || s.accountRepo == nil {
		return "", errors.New("account repository is not configured")
	}
	if !IsClaudeSKImportAccount(account) {
		return "", errors.New("account is not a Claude SK import account")
	}
	upstreamMsg := extractUpstreamErrorMessage(responseBody)
	if upstreamMsg != "" && !looksLikeTokenAuthenticationFailure(upstreamMsg) {
		return "", errors.New("401 does not look like token expiration or revocation")
	}
	...
	if err != nil {
		...
		if errors.As(err, &convertErr) {
			slog.Warn("account_test.sk_import_401_recovery_convert_failed",
		...
		if isClaudeSKAccountPermanentError(err) {
			...
			if setErr := s.accountRepo.SetError(ctx, account.ID, errorMsg); setErr != nil {
				slog.Warn("account_test.sk_import_401_recovery_set_error_failed", "account_id", account.ID, "error", setErr)
			} else {
				slog.Info("account_test.sk_import_401_recovery_set_error", "account_id", account.ID, "reason", logredact.RedactText(errorMsg))
			}
			...
			if setErr := s.accountRepo.SetTempUnschedulable(ctx, account.ID, until, reason); setErr != nil {
				slog.Warn("account_test.sk_import_401_recovery_set_temp_failed", "account_id", account.ID, "error", setErr)
			}
		}
		return "", err
	}
	...
	if s.claudeTokenProvider != nil && s.claudeTokenProvider.tokenCache != nil {
		if err := s.claudeTokenProvider.tokenCache.DeleteAccessToken(ctx, ClaudeTokenCacheKey(account)); err != nil {
			slog.Warn("account_test.sk_import_401_recovery_cache_delete_failed", "account_id", account.ID, "error", err)
		}
	}
	if err := s.accountRepo.ClearTempUnschedulable(ctx, account.ID); err != nil {
		slog.Warn("account_test.sk_import_401_recovery_clear_temp_failed", "account_id", account.ID, "error", err)
	}
	if account.Status == StatusError {
		if err := s.accountRepo.ClearError(ctx, account.ID); err != nil {
			slog.Warn("account_test.sk_import_401_recovery_clear_error_failed", "account_id", account.ID, "error", err)
		}
	}
	if !account.Schedulable {
		if err := s.accountRepo.SetSchedulable(ctx, account.ID, true); err != nil {
			slog.Warn("account_test.sk_import_401_recovery_schedulable_failed", "account_id", account.ID, "error", err)
		}
	}

	slog.Info("account_test_sk_import_401_recovered", "account_id", account.ID)
	return converted.AccessToken, nil
}
