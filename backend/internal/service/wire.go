...
// ProvideRateLimitProbeRunnerService creates and starts the half-open
// rate-limit probe runner (disabled by default; gated by config).
func ProvideRateLimitProbeRunnerService(
	accountRepo AccountRepository,
	accountTestSvc *AccountTestService,
	rateLimitSvc *RateLimitService,
	cfg *config.Config,
) *RateLimitProbeRunnerService {
	lister, _ := accountRepo.(rateLimitProbeCandidateLister)
	svc := NewRateLimitProbeRunnerService(lister, accountTestSvc, rateLimitSvc, cfg)
	svc.Start()
	return svc
}
...
func ProvideSettingService(settingRepo SettingRepository, groupRepo GroupRepository, proxyRepo ProxyRepository, cfg *config.Config) *SettingService {
	...
	SetConvertCookieProvider(settingRepo)
	SetAnthropicDefaultBaseRPMProvider(settingRepo)
