...
func (s *GatewayService) buildCountTokensRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token, tokenType, modelID string, mimicClaudeCode bool) (*http.Request, []byte, error) {
	if account.IsAnthropicOAuthOrSetupToken() {
		body = ensureClaudeOAuthIdentityPrompt(body)
	}
	body = stripDeferredToolCacheControl(body)
	// 确定目标 URL
	targetURL := claudeAPICountTokensURL
	...
	// Count the same identity sentence and caller prompts as message forwarding.
	...
	if frozenProfile != nil {
		...
		finalBetaHeader = mergeAnthropicBetaDropping(requiredBetas, finalBetaHeader, ctEffectiveDropSet)
