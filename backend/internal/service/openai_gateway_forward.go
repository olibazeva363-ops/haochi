...
func (s *OpenAIGatewayService) buildUpstreamRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token string, isStream bool, promptCacheKey string, isCodexCLI bool) (*http.Request, error) {
	...
	// 收敛为 opt-in：未启用（上下文无预计算 IDs）的账号回落到浏览器 UA 兜底，
	// 避免 Cloudflare 对浏览器型 UA 在 ChatGPT 内部接口上触发 JS 质询。
	appliedCodexConvergence := false
	if account.Type == AccountTypeOAuth && c != nil {
		if fpIDs, ok := c.Get("codex_fingerprint_ids"); ok {
			if ids, ok := fpIDs.(*codexFingerprintIDs); ok {
				...
				appliedCodexConvergence = true
			}
		}
	}
	if !appliedCodexConvergence {
