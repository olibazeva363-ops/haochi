...
type GatewaySchedulingConfig struct {
	...
	// 限流号半开重探：带外主动探测仍在冷却中的账号，成功即提前清除限流并出池，
	// 解决「上游冷却时间过于保守 / 偶发 429」导致账号额度没用完却长时间被封的问题。
	RateLimitProbe RateLimitProbeConfig `mapstructure:"rate_limit_probe"`
}

// RateLimitProbeConfig 控制限流账号的半开重探行为。默认关闭，需显式开启灰度。
type RateLimitProbeConfig struct {
	// 是否启用半开重探
	Enabled bool `mapstructure:"enabled"`
	// worker 扫描周期（秒）
	IntervalSeconds int `mapstructure:"interval_seconds"`
	// 只探测「剩余冷却时间 > 该分钟数」的账号；短冷却本就快恢复，不值得探测
	MinCooldownRemainingMinutes int `mapstructure:"min_cooldown_remaining_minutes"`
	// 同一账号两次探测之间的最小间隔（分钟），内存节流
	ReprobeIntervalMinutes int `mapstructure:"reprobe_interval_minutes"`
	// 单轮并发探测的最大 worker 数
	MaxWorkers int `mapstructure:"max_workers"`
	// 单轮最多取多少个候选账号
	BatchLimit int `mapstructure:"batch_limit"`
	// 参与探测的平台列表，默认仅 anthropic
...
func setDefaults() {
	...
	// 限流号半开重探（默认关闭，需显式灰度开启）
	viper.SetDefault("gateway.scheduling.rate_limit_probe.enabled", false)
	viper.SetDefault("gateway.scheduling.rate_limit_probe.interval_seconds", 60)
	viper.SetDefault("gateway.scheduling.rate_limit_probe.min_cooldown_remaining_minutes", 30)
	viper.SetDefault("gateway.scheduling.rate_limit_probe.reprobe_interval_minutes", 30)
	viper.SetDefault("gateway.scheduling.rate_limit_probe.max_workers", 5)
	viper.SetDefault("gateway.scheduling.rate_limit_probe.batch_limit", 200)
	viper.SetDefault("gateway.scheduling.rate_limit_probe.platforms", []string{"anthropic"})
...
func (c *Config) Validate() error {
	...
	if c.Gateway.Scheduling.RateLimitProbe.Enabled {
		probe := c.Gateway.Scheduling.RateLimitProbe
		if probe.IntervalSeconds <= 0 {
			return fmt.Errorf("gateway.scheduling.rate_limit_probe.interval_seconds must be positive")
		}
		if probe.MinCooldownRemainingMinutes < 0 {
			return fmt.Errorf("gateway.scheduling.rate_limit_probe.min_cooldown_remaining_minutes must be non-negative")
		}
		if probe.ReprobeIntervalMinutes <= 0 {
			return fmt.Errorf("gateway.scheduling.rate_limit_probe.reprobe_interval_minutes must be positive")
		}
		if probe.MaxWorkers <= 0 {
			return fmt.Errorf("gateway.scheduling.rate_limit_probe.max_workers must be positive")
		}
		if probe.BatchLimit <= 0 {
			return fmt.Errorf("gateway.scheduling.rate_limit_probe.batch_limit must be positive")
		}
		if len(probe.Platforms) == 0 {
			return fmt.Errorf("gateway.scheduling.rate_limit_probe.platforms must not be empty when enabled")
		}
	}
	if c.Gateway.Scheduling.OutboxLagWarnSeconds > 0 &&
