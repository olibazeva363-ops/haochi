...
func (r *accountRepository) ListOAuthRefreshCandidatePage(ctx context.Context, options service.OAuthRefreshPageOptions) (*service.OAuthRefreshCandidatePage, error) {
	...
	if options.ActiveOnly {
		if options.IncludeRecoverableError {
			query += `
			AND (
				status = 'active'
				OR (
					...
					AND type = 'oauth'
					AND extra->>'source' = 'sk_import'
					AND credentials ? 'sk_cookie'
					AND btrim(credentials->>'sk_cookie') <> ''
				)
			)`
		} else {
			query += `
...
// ListRateLimitedAccountsForProbe 返回「仅因限流冷却而不可调度、且剩余冷却时间超过
// minCooldownRemaining」的 active+schedulable 账号。除 rate_limit_reset_at 的判定被反转
// （> now+minCooldownRemaining，即仍在冷却且还有较长时间）外，其余条件与
// schedulableAccountsQuery 一致：账号本可调度，唯独卡在限流上。按最早恢复时间排序，
// 便于优先重探即将恢复的账号。只读查询，不修改任何账号状态。
func (r *accountRepository) ListRateLimitedAccountsForProbe(ctx context.Context, platforms []string, minCooldownRemaining time.Duration, limit int) ([]service.Account, error) {
	now := time.Now()
	if minCooldownRemaining < 0 {
		minCooldownRemaining = 0
	}
	threshold := now.Add(minCooldownRemaining)
	q := r.client.Account.Query().
		Where(
			dbaccount.StatusEQ(service.StatusActive),
			dbaccount.SchedulableEQ(true),
			...
			dbaccount.RateLimitResetAtGT(threshold),
		).
		Order(dbent.Asc(dbaccount.FieldRateLimitResetAt))
	if len(platforms) > 0 {
