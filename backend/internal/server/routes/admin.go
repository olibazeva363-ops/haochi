...
func registerAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers, stepUpAuth middleware.StepUpAuthMiddleware) {
	accounts := admin.Group("/accounts")
	{
		...
		accounts.POST("/import/sk", h.Admin.Account.ImportFromSK)
		accounts.POST("/import/sk/probe", h.Admin.Account.ProbeSKConverter)
		// 转换站默认 Cookie 管理(落库,立即生效,无需重启容器)
		accounts.GET("/import/sk/cookie", h.Admin.Setting.GetConvertCookieStatus)
		accounts.PUT("/import/sk/cookie", h.Admin.Setting.UpdateConvertCookie)
		accounts.DELETE("/import/sk/cookie", h.Admin.Setting.ClearConvertCookie)
