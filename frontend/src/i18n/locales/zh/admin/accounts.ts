export default {
    accounts: {
      ...
      skImport: '导入 SK',
      skImportTitle: '导入 Claude SK',
      skImportHint: '粘贴一个或多个 sk-ant-sid，系统会自动转换并导入为 Anthropic OAuth 账号。',
      skImportWarning: '该功能使用转换站 Cookie。服务器未配置 SUB2API_CONVERT_COOKIE 时，可在下方临时填写 Cookie。',
      skImportContent: 'SK 内容',
      skImportContentPlaceholder: 'sk-ant-sid02-...\nsk-ant-sid02-...',
      skImportContentHint: '已识别 {count} 个 SK',
      skImportCookie: '转换站 Cookie（可选）',
      skImportCookiePlaceholder: 'auth_token=...; auth_role=user; cf_clearance=...',
      skImportCookieHint: '留空则使用服务器保存的默认转换 Cookie(见下方),否则用你临时粘贴的这条。',
      skConvertCookieTitle: '服务器默认转换 Cookie',
      skConvertCookieHint: '把上面粘贴的 Cookie 保存为服务器默认值,导入 / 检测 / 后台自动重转都会用它,立即生效、无需重启容器。',
      skConvertCookieSave: '保存为默认',
      skConvertCookieSaving: '保存中...',
      skConvertCookieClear: '清除',
      skConvertCookieClearing: '清除中...',
      skConvertCookieSource: '当前来源',
      skConvertCookieSource_db: '已保存(数据库)',
      skConvertCookieSource_env: '环境变量 SUB2API_CONVERT_COOKIE',
      skConvertCookieSource_none: '未配置',
      skConvertURL: '转换接口',
      skConvertURLNotConfigured: '尚未配置 SUB2API_CONVERT_URL，SK 不会发送到转换接口。',
      skConvertURLConfigured: '已配置接口：{host}',
      skConvertURLLocalTrusted: '本地或私有网络接口',
      skConvertURLRemoteWarning: '远程接口：导入前请确认这是你信任的转换服务。',
      skConvertCookieExpiresAt: 'Cookie 到期',
      skConvertCookieNone: '当前没有可用的转换 Cookie,导入前请先保存一条。',
      skConvertCookieEmpty: '请先在上方粘贴要保存的 Cookie',
      skConvertCookieSaved: '默认转换 Cookie 已保存并立即生效',
      skConvertCookieSaveFailed: '保存默认转换 Cookie 失败',
      skConvertCookieCleared: '默认转换 Cookie 已清除',
      skConvertCookieClearFailed: '清除默认转换 Cookie 失败',
      skConvertCookieLoadFailed: '获取默认转换 Cookie 状态失败',
      skImportNamePrefix: '账号名前缀',
      skImportConcurrency: '并发数',
      skImportPriority: '优先级',
      skImportButton: '转换并导入',
      skImporting: '导入中...',
      skImportMissingSK: '请至少填写一个 SK',
      skImportResult: 'SK 导入结果',
      skImportResultSummary: '转换成功 {converted}，转换失败 {convert_failed}；账号创建 {account_created}，失败 {account_failed}',
      skImportSuccess: 'SK 导入完成：账号 {account_created}',
      skImportCompletedWithErrors: 'SK 导入完成但有错误：转换失败 {convert_failed}，账号失败 {account_failed}',
      skImportFailed: 'SK 导入失败',
      ...
      oauth: {
        ...
        claudeCredentialsAuth: '粘贴 OAuth 凭证',
        claudeCredentialsLabel: 'Claude OAuth JSON',
        claudeCredentialsPlaceholder: '粘贴 .claude/.credentials.json 内容或 OAuth token JSON',
        claudeCredentialsImport: '导入 OAuth 凭证',
        claudeCredentialsImportFailed: 'Claude OAuth 凭证导入失败',
        claudeCredentialsErrors: {
          invalid_json: '凭证不是有效的 JSON',
          invalid_payload: '凭证必须是一个 JSON 对象',
          access_token_missing: '凭证中缺少 access_token 或 accessToken',
          expires_at_invalid: '凭证中的过期时间格式无效'
        },
        ...
        sessionKeyPlaceholder:
          '每行一个 sessionKey，例如：\nsk-ant-sid01-xxxxx...\nsk-ant-sid02-yyyyy...',
        sessionKeyPlaceholderSingle: 'sk-ant-sid01/02-xxxxx... 或 sessionKey=...',
        ...
        step6: '复制 Value 列的值',
        sessionKeyFormat: '支持 sid01/sid02 纯值、sessionKey=... 或包含 sessionKey 的完整 Cookie 字符串',
        ...
        cookieErrors: {
          ...
          CLAUDE_SESSION_REQUEST_FAILED: '连接 Claude 失败，请检查账号代理后重试',
          CLAUDE_SESSION_UPSTREAM_ERROR: 'Claude Cookie 授权服务暂时异常，请稍后重试'
        },
