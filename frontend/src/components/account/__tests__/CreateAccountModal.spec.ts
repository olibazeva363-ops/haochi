...
const OAuthAuthorizationFlowStub = defineComponent({
  ...
  emits: ['import-codex-session', 'import-codex-pat', 'import-claude-credentials'],
  template: `
    <div>
...
describe('CreateAccountModal OpenAI long-context billing', () => {
  ...
  it('imports pasted Claude credentials through the OAuth create form', async () => {
    const wrapper = mountModal()
    await wrapper.get('form#create-account-form input[type="text"]').setValue('Claude import')
    await wrapper.get('form#create-account-form').trigger('submit.prevent')
    await flushPromises()

    const flow = wrapper.getComponent(OAuthAuthorizationFlowStub)
    expect(flow.props('showClaudeCredentialsOption')).toBe(true)
    flow.vm.$emit('import-claude-credentials', JSON.stringify({
    ...
    expect(createAccountMock).toHaveBeenCalledWith(expect.objectContaining({
      ...
      extra: expect.objectContaining({ base_rpm: 15, enable_tls_fingerprint: true }),
    }))
    expect(wrapper.emitted('created')).toHaveLength(1)
    wrapper.unmount()
  })

  it('keeps the create form open when pasted Claude credentials are invalid', async () => {
    const wrapper = mountModal()
    await wrapper.get('form#create-account-form input[type="text"]').setValue('Claude import')
    await wrapper.get('form#create-account-form').trigger('submit.prevent')
    await flushPromises()
    wrapper.getComponent(OAuthAuthorizationFlowStub).vm.$emit('import-claude-credentials', '{}')
    await flushPromises()
