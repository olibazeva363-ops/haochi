...
export interface SKImportRequest {
  content?: string
  sks?: string[]
  cookie?: string
  name_prefix?: string
...
export interface SKImportResult extends AdminDataImportResult {
  converted: number
  convert_failed: number
}

export interface SKConvertProbeRequest {
  sk: string
  cookie?: string
}

export interface SKConvertProbeResult {
  ok: boolean
  kind?: string
  message: string
  details?: string
  needs_cookie_refresh: boolean
  retryable: boolean
  email?: string
  expires_at?: number
...
// ConvertCookieStatus 描述当前生效的转换站默认 Cookie(不返回原文)
export interface ConvertCookieStatus {
  configured: boolean
  source: 'db' | 'env' | 'none'
  length?: number
  email?: string
