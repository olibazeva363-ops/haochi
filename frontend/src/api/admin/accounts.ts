...
import type {
  ...
  SKImportRequest,
  SKImportResult,
  SKConvertProbeRequest,
  SKConvertProbeResult,
  ConvertCookieStatus,
  UpdateConvertCookieRequest,
...
export async function importFromSK(payload: SKImportRequest): Promise<SKImportResult> {
  const { data } = await apiClient.post<SKImportResult>('/admin/accounts/import/sk', payload, {
    timeout: 180000
  })
  return data
}

export async function probeSKConverter(payload: SKConvertProbeRequest): Promise<SKConvertProbeResult> {
  const { data } = await apiClient.post<SKConvertProbeResult>('/admin/accounts/import/sk/probe', payload, {
    timeout: 60000
  })
  return data
}
...
/**
 * 获取服务器默认转换站 Cookie 的状态(不返回原文)
 */
export async function getConvertCookieStatus(): Promise<ConvertCookieStatus> {
  const { data } = await apiClient.get<ConvertCookieStatus>('/admin/accounts/import/sk/cookie')
  return data
}

/**
 * 更新服务器默认转换站 Cookie(落库,立即生效,无需重启容器)
 */
export async function updateConvertCookie(payload: UpdateConvertCookieRequest): Promise<ConvertCookieStatus> {
  const { data } = await apiClient.put<ConvertCookieStatus>('/admin/accounts/import/sk/cookie', payload)
  return data
}

/**
 * 清除服务器默认转换站 Cookie(回退到 SUB2API_CONVERT_COOKIE 环境变量)
 */
export async function clearConvertCookie(): Promise<ConvertCookieStatus> {
  const { data } = await apiClient.delete<ConvertCookieStatus>('/admin/accounts/import/sk/cookie')
  return data
}
